...
from scrapers import AliExpressExtractor, hunt_products
...
async def hunter_search_endpoint(request: Request):
    """Busca productos en Banggood con HTTP scraping."""
    try:
        body = await request.json()
    except:
        body = {}
    keywords = body.get("keywords", "phone case")
    ...
    async with httpx.AsyncClient(headers=headers, follow_redirects=True, timeout=20.0) as client:
        ...
        html = r.text
        products = _re.findall(r'href="(https://www\\.banggood\\.com/[^"]*-p-\\d+[^"]*)"[^>]*title="([^"]+)"', html)
        results = []
        for prod_url, title in products:
            if len(results) >= max_products:
                break
            pid_m = _re.search(r"-p-(\\d+)", prod_url)
            pid = pid_m.group(1) if pid_m else "?"
            # Try to get price from product page
            price = 0.0
            try:
                pr = await client.get(prod_url.split("?")[0], timeout=10.0)
            ...
            img_m = _re.search(r'src="(https://imgaz[^"]+)"', html[html.find(prod_url):html.find(prod_url)+3000] if prod_url in html else "")
            results.append({
                "product_id": pid, "url": prod_url.split("?")[0], "title": title[:150],
                "price_usd": price, "image_url": img_m.group(1) if img_m else "",
                "source": "banggood", "platform": "banggood"
            })
        return {"added": len(results), "platform": "banggood", "keywords": keywords, "products": results, "success": True}
...
def generate_mock_sourcing_data(*args, **kwargs):
    return []
@app.get("/")
def serve_frontend():
...
def run_hunter(req: HunterRequest):
    """Ejecuta el hunter en AliExpress (CDP) y/o Banggood (headless)."""
    import subprocess, json as _json
    HUNTER_SCRIPT = os.path.join(os.path.dirname(__file__), "..", "hunter_cdp.py")
    ...
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=90, cwd=os.path.dirname(HUNTER_SCRIPT))
        
        json_path = os.path.join(os.path.dirname(HUNTER_SCRIPT), "hunter_products.json")
        products = []
...
async def get_trending_products(
    ...
    """Real Product Hunter - searches live or shows curated list.
    Pass ?q=auriculares to search with realistic sourcing data.
    Pass ?mode=curated to see the pre-built 69 niche list.
    ...
    if q and q.strip():
        ...
        for i in range(count):
            ...
            products.append({
                "name": f"{q.strip().title()} {variant} — {seller}",
                ...
                "ml_avg": ml_est,
                "img": "",
                "desc": f"{q.strip().title()} {variant} por {seller}",
                "source_url": f"https://www.aliexpress.com/wholesale?SearchText={q.strip().replace(' ', '+')}",
                "rating": rating,
                "reviews": orders,
                "store": seller,
                "source": "live-hunter",
